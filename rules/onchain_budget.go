...
func removeReqId(memo string) string {
	...
	// Remove the request ID from the memo.
	colonIdx := strings.Index(memo, ":")

	return memo[colonIdx+1:]
}
...
func (o *OnChainBudgetEnforcer) cancelPendingPayment(
	...
	return o.GetStores().Update(func(tx firewalldb.KVStoreTx) error {
		...
		uniqueReqID := formatReqId(o.GetLndConnID(), o.GetReqID())
		reqPref := fmt.Sprintf("%s:%s", onChainPending, uniqueReqID)
		...
		return store.Set(ctx, onChainPending, b)
	})
}
...
func (o *OnChainBudgetEnforcer) handlePaymentConfirmed(
	...
	return o.GetStores().Update(func(tx firewalldb.KVStoreTx) error {
		...
		err = store.Set(ctx, onChainPending, b)
		if err != nil {
			return err
		}
		...
		return store.Set(ctx, onChainSpent, b)
	})
}
...
func (o *OnChainBudgetEnforcer) getBudgetState(ctx context.Context,
	...
	spentBytes, err := store.Get(ctx, onChainSpent)
	if err != nil {
		return nil, nil, err
	}

	pendingBytes, err := store.Get(ctx, onChainPending)
	if err != nil {
		return nil, nil, err
	}
...
func (o *OnChainBudgetEnforcer) addPendingAmt(ctx context.Context,
	...
	reqPref := fmt.Sprintf("%s:%s", onChainPending, reqID)
